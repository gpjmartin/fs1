...
trino --user sa.richard.persaud --password --server=https://www.net --file $outfile --password 

outfile=out_1994.sql
sed -e 's/#CAT01#/hive/g'   -e 's/#SCH01#/tpcht/g'  -e 's/#SIZE#/sf100/g'  -e 's/#YEAR#/1994/g' $file >$outfile
trino --user sa.richard.persaud --password --server=https://swww.net --file $outfile --password 

outfile=out_1995.sql
sed -e 's/#CAT01#/hive/g'   -e 's/#SCH01#/tpcht/g'  -e 's/#SIZE#/sf100/g'  -e 's/#YEAR#/1995/g' $file >$outfile
trino --user sa.richard.persaud --password --server=https://wwww.net --file $outfile --password 
